...
#define array_new(Array_h)  \
  ...
  Array_h->arr = NULL;\
  MALLOC(sizeof(TYPE)*START_DIM, Array_h->arr, TYPE*);\
...
#define array_realloc(Array_h) \
  TYPE* new_arr = NULL;\
  MALLOC(sizeof(TYPE)*Array_h->size*2, new_arr, TYPE*);\
  for(size_t i=0;i<Array_h->nelem;i++){\
...
#define array_cpy(Array_h, POINTER, SIZE, DATA) \
  if(POINTER >= 0 && POINTER <= Array_h->size){\
    TYPE* buffer = NULL;\
    MALLOC(sizeof(TYPE)*SIZE, buffer, TYPE*);\
